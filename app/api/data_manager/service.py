...
def automapping(raw_df: DataFrame):
    column_type = dict()
...
def remove_dataset_from_database(owner: str, name: str) -> Optional[bool]:
...
def get_header_from_csv(file) -> DataFrame:
    return read_csv(file, index_col=0, nrows=0).columns.tolist()


def check_db_fullness(
    folders_mapped: Dict[str, str]
) -> tuple[bool, Dict[str, List[str]]]:
    """
    Function checks if database and files relations are corrupted.De facto, make a comparison between
    database records and files.

    :param folders_mapped: a dict with table name as keys and corresponding folder as a value.
    :return: a tuple, where the first element shows a general result, the second one is detailed.
    """
    failed = False
...
def remove_cache_db(owner):
    Sample.query.filter_by(owner=owner, is_default=1).delete()
    db.session.commit()


def remove_cache_folder(owner):
    user_space = os.path.join(current_app.config["SAMPLES_FOLDER"], owner)
    for file in os.listdir(user_space):
        if file.endswith("_default.csv"):
            os.remove(os.path.join(user_space, file))
