...
def test_get_sample(client):
    ...
    response = client.get(
        f"api/bn_manager/get_display_data/{user_name}/{net_name}/{dataset_name}/{node}"
    )
...
def test_get_equal_edges(client, bn_actions):
    ...
    response = client.get(
        "api/bn_manager/get_equal_edges",
        query_string={"names": str(["a1", "a2"]), "owner": "test"},
    )
