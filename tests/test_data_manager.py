...
def test_upload(client, app):
    ...
    correct_default = {"owner": "test", "description": "test dataset"}

    package_without_file = {
        "content": file_without_file,
        "name": "no_file.csv",
    } | correct_default
    ...
    assert (
        "test_dataset"
        in client.get(
            "api/data_manager/get_datasets", query_string={"user": "test"}
        ).json.keys()
    ), "database recording failed."
    # file system check
    assert os.listdir(os.path.join(app.config["DATASETS_FOLDER"], "test")) == [
        "test_dataset.csv"
    ], "file recording failed."

    # tear down
    client.delete(
        "api/data_manager/remove_dataset",
        query_string={"owner": "test", "name": "test_dataset"},
    )
...
def test_remove_dataset(client, app):
    rv = client.delete(
        "api/data_manager/remove_dataset",
        query_string={"owner": "test", "name": "test_dataset2"},
    )
...
def test_get_root_nodes(client, app):
    # ours
    r1 = client.get(
        "api/data_manager/get_root_nodes",
        query_string={"name": "hack", "owner": "test"},
    )
    ...
    r3 = client.get(
        "api/data_manager/get_root_nodes",
        query_string={"name": "test_dataset1", "owner": "test"},
    )
    r4 = client.get(
        "api/data_manager/get_root_nodes",
        query_string={"name": "test_dataset1", "owner": "not_user"},
    )
